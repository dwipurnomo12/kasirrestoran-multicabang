    ...
    public function getData(Request $request)
    {
        ...
        if ($user->role->role === 'administrator' || $user->role->role === 'kepala restoran') {
            if ($selectedOption == '' || $selectedOption === 'Semua Cabang') {
                ...
                $pembelians = Pembelian::with('detailPembelians')->where('cabang_id', $selectedOption)->get();
            }
        } else {
            $pembelians = Pembelian::with('detailPembelians')->where('cabang_id', auth()->user()->cabang_id)->get();
        }
